...
export default function NotificationsScreen() {
  ...
  const notices = useMemo<Notice[]>(() => {
    ...
    trips.forEach((trip) => {
      if (trip.status === 'Live') {
        items.push({ id: `live-${trip.id}`, icon: 'flash-outline', tint: colors.green, bg: '#E7F9F0', title: `${trip.name} is live now`, body: 'Your trip is underway - open the plan for today.', onPress: () => router.push(`/trip/${trip.id}`) });
      } else if (trip.status === 'Planning') {
        items.push({ id: `count-${trip.id}`, icon: 'time-outline', tint: colors.blue, bg: '#EEF3FF', title: `${trip.name} · ${countdownLabel(trip.startDate, trip.status)}`, body: `${trip.destination} · tap to open the trip.`, onPress: () => router.push(`/trip/${trip.id}`) });
      }

      if (trip.readinessDone < trip.readinessTotal) {
        items.push({ id: `ready-${trip.id}`, icon: 'checkmark-circle-outline', tint: '#B87A16', bg: '#FFF4E3', title: `Finish setting up ${trip.name}`, body: `${trip.readinessDone} of ${trip.readinessTotal} setup tasks done.`, onPress: () => router.push(`/trip/${trip.id}`) });
      }
    });
...
const styles = StyleSheet.create({
  ...
  emptyIcon: { width: 52, height: 52, borderRadius: 18, backgroundColor: '#F3F1EA', alignItems: 'center', justifyContent: 'center' },
